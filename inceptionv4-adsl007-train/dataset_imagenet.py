...
def create_dataset(dataset_path, do_train, repeat_num=1, batch_size=32):
    ...
    resize_op = C.Resize(image_length+50)
    centercrop_op = C.CenterCrop(image_length)
    random_crop_op = C.RandomCrop(image_length)
    color_adj_op = C.RandomColorAdjust(0.125, 0.5, 0.5, 0.2)
    random_rot_op = C.RandomRotation(45)
    normalize_op = C.Normalize([123.6, 116.3, 103.5], [58.4, 57.1, 57.4])
    change_swap_op = C.HWC2CHW()

    if do_train:
        trans = [random_horizontal_flip_op, random_rot_op, resize_op,
                 random_crop_op, color_adj_op, normalize_op, change_swap_op]
    else:
        trans = [resize_op, centercrop_op, normalize_op, change_swap_op]
