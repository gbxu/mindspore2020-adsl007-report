"""replaced _eval_process function, make it support 144crop evaluation"""
...
class Model:
    ...
    def __eval_process(self, valid_dataset, list_callback=None, cb_params=None):
        """
        Evaluation. The data would be passed to network directly.
        ...
        dataset_helper, _ = self._exec_preprocess(self._eval_network,
                                                  ...
                                                  dataset=valid_dataset,
                                                  dataset_sink_mode=False)
        for next_element_ in dataset_helper:
            cb_params.cur_step_num += 1
            list_callback.step_begin(run_context)
            # generate 144 crop images from one image
            next144_1=[]
            next144_2=[]
            next144_3=[]
            next144_4=[]
            next144_5=[]
            next_element=next_element_[0]
            next_label=next_element_[1]
            next_element = next_element.asnumpy()
            next_element = next_element[0]
            next_element *= (0.5,0.5,0.5)
            next_element += (0.5,0.5,0.5)
            next_element *= 255
            next_element=next_element.astype(np.uint8)
            next_element=Image.fromarray(next_element)
            ratio = next_element.height/next_element.width
            if ratio > 1:
                next144_1.append(next_element.resize((352,int(352*ratio))))
                next144_1.append(next_element.resize((320,int(320*ratio))))
                next144_1.append(next_element.resize((384,int(384*ratio))))
                next144_1.append(next_element.resize((448,int(448*ratio))))
                for img in next144_1:
                    next144_2.append(img.crop((0,0,img.width,img.width)))
                    next144_2.append(img.crop((0,(img.height-img.width)//2,img.width,(img.height-img.width)//2+img.width)))
                    next144_2.append(img.crop((0,img.height-img.width,img.width,img.height)))
            else :
                next144_1.append(next_element.resize((int(352/ratio),352)))
                next144_1.append(next_element.resize((int(320/ratio),320)))
                next144_1.append(next_element.resize((int(384/ratio),384)))
                next144_1.append(next_element.resize((int(448/ratio),448)))
                for img in next144_1:
                    next144_2.append(img.crop((0,0,img.height,img.height)))
                    next144_2.append(img.crop(((img.width-img.height)//2,0,(img.width-img.height)//2+img.height,img.height)))
                    next144_2.append(img.crop((img.width-img.height,0,img.width,img.height)))
            for img in next144_2:
                next144_3.append(img)
                next144_3.append(img.transpose(Image.FLIP_LEFT_RIGHT))
            for img in next144_3:
                w=img.width
                next144_4.append(img.crop((0,0,299,299)))
                next144_4.append(img.crop((0,w-299,299,w)))
                next144_4.append(img.crop(((w-299)//2,(w-299)//2,(w-299)//2+299,(w-299)//2+299)))
                next144_4.append(img.crop((w-299,0,w,299)))
                next144_4.append(img.crop((w-299,w-299,w,w)))
                next144_4.append(img.resize((299,299)))
            for img in next144_4:
                img = np.array(img)
                img = img - (123.6, 116.3, 103.5)
                img = img / (58.4, 57.1, 57.4)
                img = np.transpose(img,(2,0,1))
                img = img[np.newaxis,:]
                next144_5.append((Tensor(img, dtype=mindspore.float32), next_label))
            for i,_next_element in enumerate(next144_5):
                output = self._eval_network(*_next_element)
                if i == 0:
                    outputs = output[1]/len(next144_5)
                    loss = output[0]/len(next144_5)
                else:
                    outputs += output[1]/len(next144_5)
                    loss += output[0]/len(next144_5)
                label = output[2]
            output_ = (loss,outputs,label)
            print(f"output_={loss, label}")
            cb_params.net_outputs = output_
            list_callback.step_end(run_context)
            self._update_metrics(output_)
    ...
    def eval(self, valid_dataset, callbacks=None, dataset_sink_mode=True):
        ...
        with _CallbackManager(callbacks) as list_callback:
            ...
            return self.__eval_process(valid_dataset, list_callback, cb_params)
